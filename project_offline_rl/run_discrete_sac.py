...
def run_discrete_sac(learning_rate, gamma, target_update_interval, batch_size, graph=False):
    ...
    name = 'SAC_marioai_%s_%s_%s_%s_%s_%s' % (level_paths.level_name, gamma, learning_rate, target_update_interval, n_epochs, batch_size)
    model_file = init_dir / pathlib.Path("data", "models", name + ".pt")
    currentMax = -100000
    ddqn_max = copy.deepcopy(sac)
...
if __name__ == '__main__':
    run_discrete_sac(training_params.learning_rate, training_params.gamma, training_params.target_update_interval, training_params.batch_size)
